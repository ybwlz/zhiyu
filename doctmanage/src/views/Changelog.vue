...
const versions = [
  {
    version: '1.12.0', date: '2026-08-15', latest: true,
    summary: '批注升级为文档块 + AI 体验集中修复：AI 真流式不吞字、diff 红绿渲染、LaTeX 公式转义、图片可删可移、托盘菜单完善。',
    sections: {
      feat: [
        { title: '批注成为文档块', desc: '批注、图片、手绘统一为文档块模型（正文只留锚点，内容独立保存），Markdown 重渲染时会复用原有批注节点——textarea、画布、光标和输入状态不再被刷新销毁。' },
        { title: '批注内可加图片', desc: '批注里点「🖼 图片」直接插入图片，与文字、手绘同框共存；图片存到批注记录，不污染正文 Markdown。' },
        { title: '阅读页批注可见', desc: '笔记阅读页 / 阅览室中批注默认展开显示（文字、图片、手绘），不再只剩一个 ① 角标。' },
        { title: '批注块可移动', desc: '批注框提供 ↑↓ 按钮，把批注与其相邻正文块交换位置，随时调整批注在笔记里的顺序。' },
        { title: '公开笔记手绘层公开', desc: '公开笔记的手绘/批注随笔记一起公开，任何人在阅读页 / 阅览室都能看到作者的字迹；私密笔记仍仅自己可见。' },
      ],
      improved: [
        { title: '批注手绘不再断触', desc: '每个批注块拥有独立的笔迹会话：已有笔迹与新笔迹叠加绘制，不再被「全页手绘」状态覆盖清空；触控绘制锁定手势，笔迹连续不断点。' },
        { title: '批注涂鸦边界裁剪', desc: '手绘严格限制在批注可视边框内，pointer 移出框外也不落笔出界。' },
        { title: '正文图片等比缩放', desc: '阅览室 / 阅读页正文图片自动等比缩放适配宽度，大图不再撑破版面（平板同样生效）。' },
        { title: '批注统一保存', desc: '保存笔记时先把所有批注文字落库再保存正文，防抖没跑完也不会丢字。' },
        { title: '批注撤销 / 重做（Ctrl+Z / Ctrl+Y）', desc: '批注的新增、删除、移动、加图、手绘都可撤销/重做；保存笔记后撤销历史清空（保存即提交）。' },
        { title: '涂鸦增量绘制', desc: '画笔画到哪只追加新线段，不再每帧清屏重画全部笔迹——笔画多、点数多也不卡。' },
        { title: '触控笔 / 手指 / 鼠标区分', desc: '触控笔与鼠标默认书写；手指默认用于滚动/平移页面，不再一碰就画脏；涂鸦面板可开启「👆 手指也绘制」用手指画。' },
      ],
      fixed: [
        { title: '修复 AI 流式最后才一次性输出', desc: 'AI 回复改为真正的逐字打字机输出，思考过程也逐字滚动，不再「三个点转半天、最后整段瞬间吐出」。' },
        { title: '修复 AI diff 顶部显示 N 处但编辑区看不到红绿', desc: '编辑器整页重渲染会抹掉已内嵌的 AI 修改块，导致只显示「已找到 N 处修改」却看不到红绿 diff；现在重渲染后自动重新内嵌。' },
        { title: '修复「去编辑页查看修改」不跳转', desc: '笔记阅读页点击 AI 修改入口不再无反应；同时 AI 助手在阅读页能正确拿到当前笔记上下文。' },
        { title: '修复 LaTeX 公式显示为代码', desc: 'AI 写入的 LaTeX 反斜杠转义错乱（如 \\cdot 被当成非法 JSON、\\frac 被吞反斜杠）导致公式不渲染；已做双反斜杠规范化 + 工具参数宽松解析。' },
        { title: 'AI 修改后不再重复输出全文', desc: 'AI 用工具改完笔记只简短说明改了什么、到编辑器确认，不再把整篇内容再输出一遍。' },
        { title: '图片插入后可下移/回车/选中删除', desc: '编辑区图片插入后光标可移到图片下方、回车换行、Backspace/Delete 删除、点击选中整张图。' },
        { title: '修复 PC 端批注图片上传报错', desc: '后端补上 img_path 更新分支，PC 端上传批注图片不再报 nothing_to_update。' },
        { title: '修复批注手绘被覆盖丢失', desc: '多次分批在手绘后保存，旧笔迹与新笔迹正确合并累积，不再被后一次保存覆盖。' },
      ],
    },
  },
  {
    version: '1.11.0', date: '2026-08-15', latest: false,
    summary: '安装器/卸载器全面改用 C++：体积 180MB → 26MB，双击秒开，卸载独立化。',
    sections: {
      new: [
        { title: '全新 C++ 安装器', desc: '用 C++ + Direct2D 重写安装向导，单文件 exe 仅 26MB（原 180MB），双击秒开、无需安装 .NET；毛玻璃界面、功能预览、主题选择、进度条、条款弹窗 100% 复刻原版。' },
        { title: '独立 C++ 卸载器', desc: '安装目录下生成「知屿卸载.exe」，双击直接进入卸载流程（无需重开安装器）；四页流程：卸载确认 → 挽留页（再想想 / 改用网页版 / 仍要卸载）→ 进度条 → 完成。' },
      ],
      improved: [
        { title: '卸载进度条', desc: '卸载过程显示与安装一致的进度条（结束进程 → 删快捷方式 → 清注册表 → 删程序文件），不再弹窗。' },
      ],
      fixed: [
        { title: '卸载更彻底', desc: '卸载范围覆盖程序文件、桌面快捷方式、注册表卸载项、开机自启；可选同时删除本地个人数据。' },
      ],
    },
  },
  {
    version: '1.10.0', date: '2026-08-14', latest: false,
  ...
  },
  {
    ...
    sections: {
      new: [
        { title: '全新自绘安装器', desc: 'Electron 自绘安装向导：毛玻璃界面、功能预览、主题选择（深空星际/晴空/简约）、安装目录可浏览、服务条款可读、流动进度条、开机自启勾选——告别传统安装器。' },
        { title: '桌面端登录态持久', desc: '修复桌面版重开掉登录：本地服务固定端口，登录一次长期保持。' },
        { title: '清空聊天记录', desc: '私信会话与 AI 助手均可一键清空对话（弹窗确认，不可恢复）。' },
        { title: '桌面端开机自启动', desc: '设置页新增「开机自启动」开关；安装器安装时可勾选。' },
      ],
      improved: [
        { title: '消息通知更及时', desc: '登录后立即加载未读数（不再要点开铃铛才显示）；点通知改路由跳转，不再整页刷新卡顿。' },
        { title: '私信会话头部优化', desc: '点头像即进主页，PC 头像靠左、移动端居中，右上角清空按钮。' },
        { title: '管理后台侧边栏按权限过滤', desc: '协管员只显示有权限的模块，备案管理仅站长可见（后端同步拦截）。' },
        { title: '用户主页跳转兼容', desc: '关注/粉丝列表进主页修复：兼容数字 id / 用户名链接。' },
      ],
    },
  },
  {
    version: '1.8.0', date: '2026-08-12', latest: false,
    summary: '桌面版体验升级：应用内自动更新、一键安装包、官网域名上线。',
    sections: {
      new: [
        { title: '桌面版自动更新', desc: '应用内检查更新：发现新版本一键下载，支持下载进度、暂停/继续/取消（断点续传），下载完成后自动替换并重启。' },
        { title: '一键安装包', desc: 'Windows 提供「安装版」（setup.exe，含桌面图标/开始菜单/卸载）与「绿色版」（zip 解压即用）两种下载方式。' },
        { title: '自定义深色标题栏', desc: '桌面版改用深色窗口标题栏，窗口控制按钮融入导航栏，与星空主题融为一体。' },
      ],
      improved: [
        { title: '下载页升级', desc: '下载页提供安装版/绿色版双选择，附未签名软件提示说明；客户端自动隐藏下载入口（已安装状态）。' },
        { title: '官网域名上线', desc: '网站启用正式域名 www.zhiyur.cn，API 与静态资源同域部署。' },
      ],
      fixed: [
        { title: '桌面版黑屏', desc: '修复打包产物模块循环引用导致的白屏/黑屏，桌面版页面正常渲染。' },
      ],
    },
  },
  {
    version: '1.7.0', date: '2026-08-12', latest: false,
    summary: '管理后台上线：用户、笔记、审核、评论、知屿币、兑换码、通知一站式管理。',
    sections: {
      new: [
        { title: '管理后台', desc: '管理员/协管员专属后台（/admin-panel），仪表盘展示用户/笔记/AI 使用量、近 7 天增长柱状图与数据分布饼图。' },
        { title: '用户管理', desc: '封禁/解封/删除用户、设为管理员、直接给用户发知屿币，支持按用户名/昵称/邮箱搜索。' },
        { title: '笔记管理', desc: '全文搜索笔记，支持编辑、置顶、删除；公开笔记自动进入审核。' },
        { title: '审核队列', desc: 'AI + 敏感词预检公开笔记，可一键通过/驳回（切换可见性）。' },
        { title: '评论管理', desc: '查看全站评论并删除不当内容。' },
        { title: '知屿币与兑换码', desc: '给指定用户发币/扣币（附备注通知）；批量生成兑换码、导出、作废，查看兑换记录。' },
        { title: '站内通知', desc: '向指定用户或全站推送通知公告。' },
        { title: '管理员权限细分', desc: '协管员可按模块勾选权限（仪表盘/用户/笔记/审核/评论/知屿币/兑换码/通知/管理员管理），只授予需要的权限。' },
      ],
      improved: [
        { title: '公开笔记自动审核', desc: '发布/更新公开笔记时自动做敏感词与 AI 审查，命中高风险自动转为待审，降低违规内容曝光。' },
      ],
      fixed: [
        { title: '仪表盘图表', desc: '修复图表不显示、重复卡片等问题；近 7 天增长以堆叠柱状图展示评论/点赞/收藏。' },
      ],
    },
  },
  {
    version: '1.6.0', date: '2026-08-12', latest: false,
    summary: '阅读滚动流畅、编辑区图片可拉伸、Markdown 能力对齐。',
    sections: {
      improved: [
        { title: '阅读页快速滚动不再白屏', desc: 'Docs 阅览室卡片去掉毛玻璃（82% 实色，观感基本不变），笔记阅读页滚动处理器节流 + 大纲位置缓存——快速拖动滚动条不再空白卡顿。' },
        { title: '编辑区图片可拉伸', desc: '粘贴/附件/复制的普通图片也自动带 8 个拉伸手柄，与「插入图片」一致；四边上下左右、四角斜向拉伸光标稳定（拉伸期间全局保持，鼠标移出图片也不变回 I 形）；图片本体为移动箭头光标。' },
        { title: '无语言代码块自动高亮', desc: '阅览室对未标注语言的代码块自动检测高亮（与笔记阅读页一致），星空主题下不再显示看不清的白色文字。' },
        { title: 'Markdown 能力对齐', desc: '笔记阅读页与编辑区补齐 Docs 已有能力：代码组 ::: code-group（tabs 切换）、代码块行高亮 {n}、行号 :line-numbers；编辑区保存时容器/警报/代码组语法完整还原。' },
        { title: '自定义容器与警报深色适配', desc: '深空星际主题下代码高亮改用 GitHub Dark 配色、GitHub 警报深色化，容器/警报/代码块在星空背景下清晰可读。' },
        { title: '笔记广场置顶补齐', desc: '热门榜/最新收录/收藏列表置顶笔记排最前并显示 📌 标记。' },
      ],
      fixed: [
        { title: '笔记广场快速下滑空白', desc: '列表图片去掉懒加载并加占位背景，快速滚动不再白屏。' },
        { title: '带尺寸参数的图片不显示', desc: '修复提取图片链接时被「=宽x高」尺寸参数打断的问题，带尺寸的图片（如编辑区插入后复制的图）在广场/预览正常显示；加载失败的坏图自动隐藏。' },
        { title: '文档更新', desc: 'docs/markdownSE.md 重写为「知屿 Markdown 语法说明」，只记录知屿实际支持的语法并明确列出不支持的 VitePress 特性。' },
      ],
    },
  },
  {
    version: '1.5.0', date: '2026-08-12', latest: false,
    summary: '安全加固与体验完善：上传/价格/限流修复、附件下载补全、消息通知可删除。',
    sections: {
      improved: [
        { title: '消息通知支持删除', desc: '通知铃铛下拉每条右侧新增「✕」删除按钮（悬停显示），也支持右键直接删除该条；删除后即时从列表移除并同步未读数。' },
        { title: '笔记阅读页附件下载', desc: '笔记广场打开笔记的阅读页新增「📎 附件」按钮：上传过附件的笔记可直接下载/查看附件（此前仅在阅览室可下载）。' },
        { title: '验证码发送更安全', desc: '验证码不再无条件随接口返回：仅本地开发且未配置邮箱时才回显 dev_code，一旦配置了 SMTP（生产环境）即使开发模式也严格不返回，杜绝「任意邮箱注册任意账号」的漏洞。' },
        { title: 'AI 额度统一', desc: 'AI 私信与 AI 助手共用同一免费额度，超限提示可去知屿币商城兑换，防止开放注册后被刷 API 额度。' },
      ],
      fixed: [
        { title: 'CORS 白名单修复', desc: '修复第二次 CORS(app) 调用覆盖白名单、实际允许所有来源的问题，现在仅放行前端站点跨域。' },
        { title: '笔记价格与上传限制', desc: '笔记价格禁止为负（创建/修改/购买三端校验，堵住负价刷知屿币）；上传改用真实大小校验 + 附件扩展名白名单 + 请求体上限，头像限 2MB、封面限 5MB。' },
        { title: '登录与验证码防护', desc: '密码登录失败 5 次锁定 10 分钟、验证码发送同 IP 限流；验证码比较改常量时间比较；大写用户名可正常登录。' },
        { title: '知屿币每日上限', desc: '点赞/收藏/转发给作者的知屿币收益增加每日上限（30/30/20），防小号互刷。' },
        { title: '其他安全项', desc: '批注贴图校验真实图片格式、下载笔记需登录、删除过期 token、安全响应头（防内容嗅探/点击劫持）、笔记计数列名白名单。' },
      ],
    },
  },
  {
    version: '1.4.0', date: '2026-08-12', latest: false,
    summary: '登录页改版：邮箱验证码登录即注册（无独立注册流程），密码登录保留。',
    sections: {
      improved: [
        { title: '邮箱验证码登录即注册', desc: '登录页新增「邮箱登录」：输入邮箱收验证码即可登录，首次使用自动创建账号（昵称默认取邮箱前缀），无需单独注册流程；老用户可切换「密码登录」（邮箱或原用户名）。' },
        { title: '登录流程精简', desc: '注册字段从 6 项减到 2 项；首次建号须勾选《服务条款》和《隐私协议》（默认不勾选，未勾选会被拦截），老用户登录不受影响。' },
      ],
    },
  },
  {
    version: '1.3.0', date: '2026-08-12', latest: false,
